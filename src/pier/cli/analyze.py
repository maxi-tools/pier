...
from pier.cli.utils import run_async
...
def check_command(
    ...
    from pier.analyze.checker import run_check
...
def analyze_command(
    ...
    from pier.analyze.models import JobAnalyzeResult
    ...
    try:
        if _is_job_dir(path):
            ...
            console.print(
                ...
                f"(no job.log). "
                f"For task quality checks, use 'pier check <task-dir>' instead.[/red]"
            )
            raise typer.Exit(1)
    except (FileNotFoundError, ValueError, RuntimeError) as e:
        console.print(f"[red]❌ {e}[/red]")
