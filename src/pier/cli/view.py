...
def view_command(
    ...
    Example usage:
        pier view ./my-tasks          # auto-detects tasks
        pier view ./my-jobs            # auto-detects jobs
        pier view ./tasks --tasks      # force tasks mode
        pier view ./jobs --port 9000
        pier view ./jobs --dev
    """
...
def _run_production_mode(
    ...
    *,
    ...
    from pier.viewer import create_app
    ...
    console.print("[green]Starting Pier Viewer[/green]")
...
def _run_dev_mode(
    ...
    folder_label = "Tasks folder" if mode == "tasks" else "Jobs folder"
    console.print("[green]Starting Pier Viewer (dev mode)[/green]")
    console.print(f"  {folder_label}: {folder}")
    console.print(f"  Mode: {mode}")
    ...
    os.environ["PIER_VIEWER_FOLDER"] = str(folder)
    os.environ["PIER_VIEWER_MODE"] = mode

    # Run backend - uvicorn handles SIGINT/SIGTERM and shuts down gracefully
    try:
        uvicorn.run(
            "pier.viewer:create_app_from_env",
            host=host,
            port=backend_port,
            log_level="info",
            reload=True,
